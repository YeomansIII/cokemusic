...
var IgeAnimationComponent = IgeEventingClass.extend({
	...
	setFrame: function(animId, frameIndex) {
		if (this._anims) {
			var anim = this._anims[animId];
			if (anim) {
				if(anim.frames[frameIndex]) {
					//Grab the selected frame and create a temp animation for it.
					...
					this.log('Cannot set frame to "' + animId + '" because the frame does not exist inside the animation!', 'warning');
				}
			} else {
				this.log('Cannot set frame to "' + animId + '" because the animation does not exist!', 'warning');
			}
		} else {
			this.log('Cannot set frame to "' + animId + '" because no animations have been defined with defineAnim(...);', 'warning');
		}
	},
	...
	/**
	 * Clears all previously defined animations.
	 */
	clearAll: function() {
		this._anims = [];
	},
